...
const CashOutRequest = () => {
  ...
  useEffect(() => {
    const fetchCashInRequests = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/cashout-requests', {
  ...
  const approveRequest = async (requestId) => {
    try {
      const token = localStorage.getItem('access-token');
      await axios.post('http://localhost:5000/cashout/approve', { requestId }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className='w:50% md:max-w-4xl ml-0 md:ml-64 mx-auto p-4'>
      <h1 className='text-center bg-black font-serif text-white p-3 text-2xl md:text-4xl'>Cash-Out Requests</h1>
      {loading ? (
       <LoadingSpinner/>
      ) : error ? (
